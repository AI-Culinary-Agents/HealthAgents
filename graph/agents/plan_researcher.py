...
def research_plan_node(state):
    from graph import AgentState, model, Queries, tavily, add_history  # avoid circular import
    ...
    if state["use_saved_data"] and os.path.exists(filename):
        ...
        with open(filename, 'r') as file:
            ...
            add_history(state, "researcher", saved_data["content"])
            return {"content": saved_data["content"], "history": state["history"]}
    
    # Generate queries based on the plan and task
    ...
    # Execute queries and append results to content
    ...
    # Update history with the research results
    add_history(state, "researcher", content)
    
    state['search_number'] += 1
    print("-----RESEARCHER------")
    
    return {"content": content, "history": state["history"]}
